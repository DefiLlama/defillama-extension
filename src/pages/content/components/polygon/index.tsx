...
import { POLYGON_SELECTORS } from "@src/pages/libs/selectors";

import gib from "@src/assets/img/memes/gib-128.png";

logImage(gib, "Llama Power activated on PolygonScan");
...
async function renderMissingPricesInDropdownOnAddressPage() {
  const listItems = document.querySelectorAll<HTMLAnchorElement>(POLYGON_SELECTORS.address.tokenList.select);
  const listItemsMap = Array.from(listItems).reduce((acc, item) => {
    const url = new URL(item.href);
    const address = url.pathname.split("/")[2];
    const prefixedAddress = "polygon:" + address;
...
async function renderErc20PriceOnAddressPage() {
  const { price, symbol } = (await getTokenPrice("polygon:" + account)) ?? {};
  if (!price) {
    console.log("Llama doesn't know the price of this token");
    return;
  }
  if (!document.querySelector(POLYGON_SELECTORS.address.erc20.test)) {
    console.log("Llama thinks this is not an ERC20 token");
    return;
  }
  if (document.querySelector(POLYGON_SELECTORS.address.erc20.select)) {
    console.log("Llama thinks Etherscan already has the price");
    return;
  }

  const sibling = document.querySelector(POLYGON_SELECTORS.address.erc20.appendTo);
